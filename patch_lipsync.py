...
# 2. Reset states on run
content = content.replace(
    "setProgressStep(0);\n    const stepsList = getSteps();",
    "setProgressStep(0);\n    setProgressPercent(0);\n    const stepsList = getSteps();"
)

# 3. Add percentage interval
interval_code = """
...
"""
if old_interval.strip() in content:
    content = content.replace(old_interval.strip(), interval_code.strip())
else:
    # try regex
    content = re.sub(r'const stepInterval = setInterval\(\(\) => \{.*?\}, 4500\);', interval_code.strip(), content, flags=re.DOTALL)
