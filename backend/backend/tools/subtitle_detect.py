...
class SubtitleDetect:
    ...
    def detect_subtitle(self, img):
        ...
        # Optimization: If manual sub_areas are provided, skip OCR entirely!
        if has_areas:
            for s_ymin, s_ymax, s_xmin, s_xmax in sub_areas:
                temp_list.append((s_xmin, s_xmax, s_ymin, s_ymax))
            return temp_list
    ...
    def find_subtitle_frame_no(self, sub_remover=None):
        ...
        # Optimization: If manual sub_areas are provided, every frame has this subtitle.
        if self.sub_areas and len(self.sub_areas) > 0:
            ...
            new_subtitle_frame_no_box_dict = {}
            for i in range(1, frame_count + 1):
                if is_frame_number_in_ab_sections(i - 1, sub_remover.ab_sections if sub_remover else None):
                    new_subtitle_frame_no_box_dict[i] = temp_list
            return new_subtitle_frame_no_box_dict

        # Original logic for OCR detection (not shown, to be replaced by just returning empty dict if we don't care about OCR)
        # We assume the user always provides sub_areas in our app.
        raise Exception("Automatic subtitle detection (OCR) is disabled. Please provide subtitle areas manually.")
